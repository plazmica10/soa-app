...
curl -s http://localhost:8080/health && echo " ✅ Gateway is healthy" || echo " ❌ Gateway is not responding"

echo "Testing Blog Service health..."
curl -s http://localhost:8081/health && echo " ✅ Blog Service is healthy" || echo " ❌ Blog Service is not responding"

echo "Testing Prometheus..."
curl -s http://localhost:9090/-/healthy && echo " ✅ Prometheus is healthy" || echo " ❌ Prometheus is not responding"

echo "Testing Jaeger..."
curl -s http://localhost:16686/ > /dev/null && echo " ✅ Jaeger is responding" || echo " ❌ Jaeger is not responding"
